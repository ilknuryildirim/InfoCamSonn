 ...
 */
package com.infocam.data.convert;
...
import com.infocam.POIMarker;
import com.infocam.data.DataHandler;
import com.infocam.data.DataSource;
import com.infocam.lib.HtmlUnescape;
import com.infocam.data.convert.DataProcessor;
...
public class WikiDataProcessor extends DataHandler implements DataProcessor{
	...
	public boolean matchesRequiredType(String type) {
		if(type.equals(DataSource.TYPE.WIKIPEDIA.name())){
			Log.v("unique1", "wikidataprocessor matches require type");
			return true;
		}
		return false;
	}
	...
	public List<Marker> load(String rawData, int taskId, int colour) throws JSONException {
		...
		Log.v("unique2", "wikidataprocessor loadın içinde");
		for (int i = 0; i < top; i++) {
