...
public class DataSource extends Activity {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
			final EditText nameField = (EditText) findViewById(0);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		int base = Menu.FIRST;
		menu.add(base, base, base, "cancel");
	...
	public DataSource(String name, String url, TYPE type, DISPLAY display,
		...
		Log.d("infocam", "New Datasource!" + name + " " + url + " " + type + " "
				+ display + " " + enabled);
	}
	...
	public String createRequestParams(double lat, double lon, double alt,
		...
		/*Toast.makeText(this, "DataSource param ",
				Toast.LENGTH_LONG).show();*/
		String ret = "";
		Log.v("datasource", "create req params ");
		if (!ret.startsWith("file://")) {
			switch (this.type) {

			case WIKIPEDIA:
				Log.v("WIKI", "wikipedia selected" );
			...
			case BUZZ:
				Log.v("WIKI", "wikipedia not selected" );
